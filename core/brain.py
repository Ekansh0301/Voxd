...
class QuickRouter:
    ...
    def match(self, text: str) -> Optional[dict]:
        ...
        for pat, url in [
            ...
            (
                r"(?:search|ask|type|query|write)\s+(.+?)\s+on\s+perplexity",
                "https://www.perplexity.ai",
            ),
            (
                r"open\s+chatgpt\s+(?:and\s+)?(?:ask|search|write|type)\s+(.+)",
                "https://chatgpt.com",
            ),
            ...
            if m:
                return {"action": "open_url_and_type", "url": url, "query": m.group(1).strip()}
        ...
        m = re.search(
            r"^open\s+((?:https?://)?[\w\-]+\."
            r"(?:com|org|net|in|io|co|dev|app|ai|tv|edu|gov)\S*)\s*$",
            t,
        )
        if m:
            url = m.group(1)
        ...
        if m and "on " not in m.group(1):
            q = m.group(1).strip().replace(" ", "+")
            return {"action": "open_url", "url": f"https://www.google.com/search?q={q}"}
        ...
        if re.search(
            ...
            r"|is\s+it\s+(?:raining|sunny|hot|cold|cloudy|windy)",
            t,
        ):
        ...
        if m:
            return {"action": "weather_activity", "activity": m.group(1).strip()}

        # ── Time (atomic) ─────────────────────────────────────────────────
        if re.search(
            r"(?:what(?:s|'?s|\s+is)?)\s+(?:the\s+)?(?:time|date|day)"
            r"|current\s+(?:time|date)|what\s+time\s+is\s+it",
            t,
        ):
...
def _format_for_speech(text: str, query: str = "") -> str:
    ...
    text = re.sub(r"https?://(?:www\.)?([^/\s]+)\S*", lambda m: m.group(1), text)
...
class Brain:
    ...
    # ── Public entry point ────────────────────────────────────────────────────
    def think(self, user_input: str, memory_context: str = "") -> tuple[str, list]:
        """
        ...
        if intent:
            ...
            if action == "weather_activity" and self.desktop:
                raw = self.desktop.is_good_for_activity(intent.get("activity", "going outside"))
                self._add(user_input, raw)
                return raw, []
    ...
    # ── LLM call ─────────────────────────────────────────────────────────────
    def _llm(self, user_input: str, memory_context: str) -> tuple[str, list]:
        ...
        msgs = [{"role": "system", "content": _sys(self._name, self._user, self._city)}]
    ...
    def _fallback(self, query: str) -> str:
        ...
        try:
            ...
            r.raise_for_status()
            return (
                self._clean(r.json().get("message", {}).get("content", ""))
    ...
    def synthesize(self, original_query: str, initial_response: str, tool_outputs: list) -> str:
        """
    ...
    def inject_tool_result(self, name: str, result: str):
        ...
        if result and result.strip():
            ...
            self.history.append(
                {"role": "user", "content": f"[{name} completed. Result: {spoken_result}]"}
            )
    ...
    def _maybe_summarise(self):
        ...
        compressed_text = "\n".join(
            f"{m['role'].upper()}: {m['content'][:100]}" for m in to_compress
        )
        try:
            import requests as req
            ...
            },
            {
                ...
                "function": {
                    ...
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "keys": {"type": "string", "description": "e.g. ctrl+w, alt+f4, ctrl+t"}
                        },
                        "required": ["keys"],
                    },
                },
