...
class ProactiveMonitor:
    ...
    def _check_disk(self):
        ...
        if usage.percent > 90 and self._should_alert("disk_90"):
            self.alert(
                f"Heads up — disk is {usage.percent:.0f}% full. "
                f"Only {usage.free // 1024**3}GB left."
            )
        elif usage.percent > 80 and self._should_alert("disk_80"):
            self.alert(
                f"Disk is {usage.percent:.0f}% full, " f"{usage.free // 1024**3}GB remaining."
            )
    ...
    def _check_ram(self):
        ...
        if mem.percent > 92 and self._should_alert("ram_92"):
            self.alert(
                f"RAM is critically high at {mem.percent:.0f}%. "
    ...
    def _check_cpu(self):
        ...
        if cpu > 95 and self._should_alert("cpu_95"):
            ...
            try:
                ...
                self.alert(f"CPU is maxed at {cpu:.0f}%. " f"Top process: {culprit}")
            except Exception:
            ...
            try:
                ...
                if r.stdout.strip() != "active":
                    if self._should_alert(f"svc_{svc}"):
                        self.alert(
                            f"Warning — {svc} service is not running. "
