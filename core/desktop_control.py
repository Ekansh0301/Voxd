...
APP_ALIASES = {
    ...
    'text editor': 'gedit', 'vscode': 'code', 'code editor': 'code',
...
WEATHER_CODES = {
    ...
    63: 'rain', 65: 'heavy rain', 80: 'showers', 81: 'heavy showers',
...
class DesktopControl:
    ...
    def open_url_and_type(self, url: str, query: str,
                          delay: float = 2.5) -> str:
        ...
        try:
            subprocess.run(
                ['xdotool', 'type', '--clearmodifiers', '--delay', '40', query],
                timeout=15)
            time.sleep(0.3)
    ...
    def play_youtube(self, query: str) -> str:
        ...
        return (f"Opened YouTube search for {query}. "
                f"Say 'click play' to start the first video.")
    ...
    def keyboard_shortcut(self, keys: str) -> str:
        ...
        try:
            ...
            subprocess.run(['xdotool', 'key', xkeys],
    ...
    def click_play(self) -> str:
        ...
        try:
            ...
            subprocess.run(['xdotool', 'key', 'Tab'], timeout=3,
                           capture_output=True)
            time.sleep(0.2)
            subprocess.run(['xdotool', 'key', 'Tab'], timeout=3,
                           capture_output=True)
    ...
    def get_logs(self, service: str = '', lines: int = 50) -> str:
        cmd = (f"journalctl -u {service} -n {lines} --no-pager"
               if service else f"journalctl -n {lines} --no-pager")
