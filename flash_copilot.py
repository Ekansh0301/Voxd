...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(PROJECT_DIR / "flash_copilot.log"), logging.StreamHandler()],
)
...
class AvatarWidget(QWidget):
    ...
    def _tick(self):
        ...
        if self.state == AvatarState.IDLE:
            self._pulse_scale = 1.0 + 0.06 * abs(__import__("math").sin(self._anim_tick * 0.8))

        elif self.state == AvatarState.LISTENING:
            import math

            for i in range(len(self._wave_bars)):
                target = 0.3 + 0.7 * abs(math.sin(self._anim_tick * 3 + i * 0.7))
            ...
            self._pulse_scale = 1.0 + 0.04 * abs(math.sin(self._anim_tick * 4))
    ...
    def paintEvent(self, event):
        ...
        p.drawEllipse(cx - scaled_r, cy_bounce - scaled_r, scaled_r * 2, scaled_r * 2)
        ...
        p.drawEllipse(
            cx - scaled_r - 4, cy_bounce - scaled_r - 4, (scaled_r + 4) * 2, (scaled_r + 4) * 2
        )
        ...
        if self.state == AvatarState.IDLE:
            ...
            # Spinning arcs
            p.setPen(
                QPen(QColor(255, 165, 0, 200), 3, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
            )
            p.setBrush(Qt.BrushStyle.NoBrush)
            rect = QRect(cx - 32, cy_bounce - 32, 64, 64)
            p.drawArc(rect, int(self._spin_angle) * 16, 120 * 16)
            p.setPen(
                QPen(QColor(255, 200, 50, 150), 2, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
            )
            p.drawArc(rect, int(self._spin_angle + 180) * 16, 80 * 16)
        ...
        if self._text:
            ...
            p.setFont(font)
            p.drawText(QRect(0, 210, 240, 22), Qt.AlignmentFlag.AlignCenter, self._text)

        if self._subtext:
            p.setPen(QColor(160, 160, 200, 180))
            font2 = QFont("Sans", 7)
            p.setFont(font2)
            p.drawText(QRect(0, 232, 240, 18), Qt.AlignmentFlag.AlignCenter, self._subtext[:25])
        ...
        p.setFont(font3)
        p.drawText(
            QRect(0, 252, 240, 16), Qt.AlignmentFlag.AlignCenter, state_labels.get(self.state, "")
        )
...
class ChatWindow(QWidget):
    ...
    def _setup_window(self):
        ...
        self.resize(w, h)
        self.move(screen.width() - w - 20, screen.height() - h - 60)
    ...
    def _setup_ui(self):
        ...
        title_bar.setStyleSheet("background: #1a1a2e; border-radius: 0px;")
        tb_layout = QHBoxLayout(title_bar)
        tb_layout.setContentsMargins(12, 0, 8, 0)

        self._title_label = QLabel("⚡ Flash Copilot")
        self._title_label.setStyleSheet("color: #a0a0ff; font-size: 12px; font-weight: bold;")

        self._status_label = QLabel("ready")
        self._status_label.setStyleSheet("color: #505070; font-size: 10px;")
        ...
        self._input.setPlaceholderText("Type a command or question… (Enter to send)")
        ...
        # ── Hint bar ──────────────────────────────────────────────────────
        hint = QLabel("Hold Right Ctrl to speak  ·  Ctrl+Space to toggle this window")
        hint.setStyleSheet("background:#0a0a18;color:#404060;font-size:10px;" "padding:3px 12px;")
    ...
    def add_user_message(self, text: str):
        self._chat.append(f'<p style="color:#8888ff;margin:4px 0">' f"<b>You:</b> {text}</p>")
    ...
    def add_flash_message(self, text: str, is_action: bool = False):
        ...
        self._chat.append(
            f'<p style="color:{color};margin:4px 0">' f"<b>{label}:</b> {display}</p>"
        )
        self._scroll_to_bottom()

    def add_system_message(self, text: str):
        self._chat.append(
            f'<p style="color:#606080;font-size:11px;margin:2px 0">' f"<i>{text}</i></p>"
        )
        self._scroll_to_bottom()
...
class WorkerThread(QThread):
    ...
    def _process(self, user_input: str):
        ...
        simple_cmd = bool(
            self.brain.router.match(user_input) if hasattr(self.brain, "router") else None
        )
        ...
        if tool_outputs:
            ...
            if has_data:
                final_reply = self.brain.synthesize(user_input, response, tool_outputs)
            else:
...
class TrayIcon(QSystemTrayIcon):
    ...
    def _create_menu(self):
        menu = QMenu()
        menu.addAction("Show Avatar", lambda: self.app_ref.avatar.show())
        menu.addAction("Hide Avatar", lambda: self.app_ref.avatar.hide())
        menu.addSeparator()
        menu.addAction("Chat Window  (Ctrl+Space)", self.app_ref.toggle_text_mode)
        menu.addSeparator()
...
class FlashCopilot(QObject):
    def __init__(self, config: dict):
        ...
        self.voices_dir = Path(config.get("voices_dir", str(PROJECT_DIR / "voices")))
        ...
        self.brain = Brain(model=self.model_name, desktop=self.desktop)  # inject desktop
        self.voice = VoiceEngine(voices_dir=self.voices_dir, voice_model=self.voice_model)
        self.hotkey_listener = HotkeyListener(
            hotkey=self.hotkey, on_press=self._on_hotkey_press, on_release=self._on_hotkey_release
        )

        # Proactive monitor
        self.monitor = ProactiveMonitor(alert_callback=self._on_proactive_alert)
    ...
    def launch_ui(self):
        ...
        self._chat_shortcut = QShortcut(QKeySequence("Ctrl+Space"), self.avatar)
        ...
        # Worker thread
        self.worker = WorkerThread(self.brain, self.voice, self.desktop, self.memory, self.safety)
        ...
        def transcribe_and_enqueue():
            ...
            if text and len(text.strip()) > 2:
                ...
                self.tray.showMessage(
                    "Flash heard you:", text, QSystemTrayIcon.MessageIcon.Information, 2000
                )
    ...
    def _on_proactive_alert(self, message: str):
        ...
        self.tray.showMessage("Flash Alert", message, QSystemTrayIcon.MessageIcon.Warning, 5000)
...
def load_config() -> dict:
    ...
    model = model_file.read_text().strip() if model_file.exists() else "qwen2.5-coder:3b"
    voice = voice_file.read_text().strip() if voice_file.exists() else "en_US-ryan-high"
...
def main():
    ...
    parser.add_argument("--hotkey", default=None, help="Override hotkey (e.g. ctrl_r, scroll_lock)")
    ...
    Path(config.get("data_dir", str(PROJECT_DIR / "data"))).mkdir(parents=True, exist_ok=True)
